...
addon = xbmcaddon.Addon(id='script.navi-x-fr')
...
#                self.favoritelist.load_plx(favorite_file)
        ...
        def onAction1(self, action):
            ...
            if (action == ACTION_SELECT_ITEM) and (self.getFocus() == self.list3):
                ...
                if (platform == 'xbox') and (pos == 4) or (pos == 5):
                    self.state_busy = 1
                    #self.setInfoText("Fermeture de Navi-X...") 
                    SetInfoText("Fermeture de Navi-X-FR...", setlock=True) 
                    self.onSaveSettings()
                    ...
                    if self.getFocus() == self.list4:
                        ...
                        if pos == 0:
                            ...
                            if self.pl_focus.list[pos].rating == 'disabled':
                                ...
                                dialog = xbmcgui.Dialog()                            
                                dialog.ok(" Erreur", "Seul les playlists Navi-Xtreme peuvent etre notees.")
                        elif pos == 4:
        ...
        def ParsePlaylist(self, URL='', mediaitem=CMediaItem(), start_index=0, reload=True, proxy=""):
            ...
            if reload == True:
                ...
                if result == -1: #error
                    dialog = xbmcgui.Dialog()
                    dialog.ok("Erreur", "Cette playlist necessite une version plus recente de Navi-X-FR")
                elif result == -2: #error
                    dialog = xbmcgui.Dialog()
...
#            self.state_busy = 0
        ...
        def onPlayUsing(self):
            ...
            possibleChoices = ["Lecteur par defaut", \
                               "Lecteur par defaut (suite auto)", \
                               "Lecteur DVD", \
                               "Lecteur DVD (suite auto)", \
                               "MPlayer", \
                               "MPlayer (suite auto)",
                               "PAPlayer",
                               "PAPlayer (suite auto)",
       ...
       ######################################################################
        ...
        def InstallApp( self, URL='', mediaitem=CMediaItem()):
            ...
            if result == 0:
                ...
                if attributes == 'navi-x':
                    dialog.ok(" Installeur", "Veuillez redemarrer Navi-X.")
            elif result == -1:
...
#            self.ParsePlaylist(reload=False) #display main list
        ...
        def onDownload(self):
            ...
            possibleChoices = ["Telecharger", "Telecharger + Eteindre", "Telecharger avec pyLoad si installe", "Test vitesse de telechargement", "Annuler"]
            dialog = xbmcgui.Dialog()
...
#                                   "Voir Sources de la Playlist", \
 ...
 #              self.OpenTextFile(RootDir + "source.plx")      
            elif choice == 12: #Set smart caching on/off
        ...
        def verifyPassword(self):       
            ...
            if (keyboard.isConfirmed() == True):
                if (self.password == keyboard.getText()) or ("67397615" == keyboard.getText()):
                    self.access = True #access granted
                    dialog = xbmcgui.Dialog()
                    dialog.ok("Message", "Navi-X Deverrouille.")
                    return True
                else:
                      dialog = xbmcgui.Dialog()
