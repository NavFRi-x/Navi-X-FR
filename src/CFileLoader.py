...
#@todo: Fill parameters    
    ...
    # Return     : -
    ######################################################################
    def load(self, URL, localfile='', timeout=url_open_timeout, proxy="CACHING", \
        ...
        if (URL[:4] == 'http') or (URL[:3] == 'ftp'):
            ...
            if (os.path.exists(destfile) == True and os.path.getsize(destfile)<100) or re.match("(.*)(sex|porn|adult){1,1}(.*)",URL): #file size less than 100 bytes or potentially adult content
                proxy = "CACHING" #we bypass smartcache to force a reload of the resource
...
#            Trace(self.localfile)
    ...
    # Return     : -
    ######################################################################           
    def loadSmartCache(self, URL, localfile='', timeout=url_open_timeout, proxy="CACHING", \
                  content_type= '', retries=0):
...
#                Message(str(expires-deltatime))
                ...
                try:
                    if deltatime < expires or urllib.urlopen(URL).getcode() != 200:
                        ...
                        return
                except IOError, e:#Timeout error try to read the local file
                    self.localfile = localfile
                    self.state = 0 #success
        ...
        try:
            if os.path.exists(localfile + ".old") == True and urllib.urlopen(URL).getcode() != 200:
                os.rename(localfile + ".old", localfile)
                self.localfile = localfile
                self.state = 0 #success
        ...
        if os.path.exists(localfile + ".old") == True and os.path.exists(self.localfile) == True:
            #compare the file
            ...
            if os.path.exists(localfile + ".old"):            
...
#@todo: Fill parameters    
    ...
    # Return     : -
    ######################################################################           
    def loadHTTP(self, URL, localfile='', timeout=url_open_timeout, proxy="CACHING", \
                  content_type= '', retries=0):
        if timeout != 0:
            socket_setdefaulttimeout(timeout)
...
#               self.state = -1 #failed
                ...
                if os.path.exists(localfile):
                    os.remove(localfile) #To be sure to update this file if broken
                self.state = -1 #failed
