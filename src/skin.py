...
def load_skin(window):   
    ...
    item = xbmcgui.ListItem("Accueil")   
    window.list3.addItem(item)
    item = xbmcgui.ListItem("Favoris")   
    window.list3.addItem(item)
    item = xbmcgui.ListItem("Telechargements")   
    window.list3.addItem(item) 
    if platform != 'xbox':
        item = xbmcgui.ListItem("Vue: " + window.listview)   
        window.list3.addItem(item)   
    item = xbmcgui.ListItem("Se connecter")   
    window.list3.addItem(item)    
    item = xbmcgui.ListItem("Quitter") 
    window.list3.addItem(item)
    
    item = xbmcgui.ListItem("Lire")  
    window.list4.addItem(item)
    item = xbmcgui.ListItem("Ajouter aux Favoris")  
    window.list4.addItem(item)    
    item = xbmcgui.ListItem("Telecharger") 
    window.list4.addItem(item)      
    item = xbmcgui.ListItem("Noter") 
    window.list4.addItem(item)   
    item = xbmcgui.ListItem("Plus d'Options...") 
    window.list4.addItem(item)   
